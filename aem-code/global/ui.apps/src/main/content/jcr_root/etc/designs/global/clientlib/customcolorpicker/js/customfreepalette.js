(function ($) {
  ...
  CUI.CustomColorPicker = new Class(
    /** @lends CUI.CustomColorPicker# */
    {
...
//                                console.log(e.message);
        ...
        for (var i = 0; i < this.palettePageSize; i++) {
          ...
          for (var sh = 0; sh < this.colorShadeNo; sh++) {
            if (this.options.config.displayModes.classicPalette) {
              ...
              if (this.colorNames.length - 1 < i *
                this.colorShadeNo + sh + this.lowerLimit) {
                html += "<td><a></a></td>";
              } else {
                rgb = CUI.util.color.HexToRGB(this.options.config.colors[this.colorNames[i *
                  this.colorShadeNo + sh + this.lowerLimit]]);
  ...
  CUI.Widget.registry.register("customColorPicker", CUI.CustomColorPicker);

  // Data API
  if (CUI.options.dataAPI) {
    $(document).on("cui-contentloaded.data-api", function (e) {
      CUI.CustomColorPicker.init($("[data-init~=customColorPicker]", e.target));
    });
  }
}(window.jQuery));
