...
<%
	...
	String errorMessage =  properties.get("errormessage",(String) null);
	String showLogin = properties.get("showlogin",(String) null);
...
<!--div class="title-nav text-uppercase"><fmt:message key="lbSetting"/></div-->
    <ul id="setting-menu" class="sidebar-menu">
        ...
        <!--Start of Changes for Amway Today 3.0 -->
        <%if(showlogin){%>

        <li style="display:none;" id="sea-login-site" class="has-sub main-blogging"><a href="javascript:void(0)"><fmt:message key="aboLogin"/></a><span class="explander">+</span>
            <ul class="btn-option-header login-form sub-menu" data-value="<%= LinkTransformerUtils.transformPath(slingRequest.getResourceResolver(), currentPage.getPath()) %>">                
                <form class="sea-login">
                    <li>                        
                        <input class="ibo-number-input" name="sea-mobile-ato-username" type="text" placeholder="ABO No" />                       
                    </li>
                    <li>                        
                        <input class="password-input" name="sea-mobile-ato-password" type="password" placeholder="Password" />                         
                    </li>
                </form>
                <li class="btn-login text-center"><a id="sea-moile-login-btn" href="#"><fmt:message key="btnLogin"/></a></li>               
                
            </ul>
        </li>

        <li style="display:none;" id="sea-logout-site"><a href="#" id="sea-mobile-logout-btn">Logout</a></li>
        <%}%>
        <!-- End of changes for AmwayToday 3.0-->
...
<script type="text/javascript">
   setParams('<%=country%>','<%=errorMessage%>');
     $(document).ready(function(){
         if(($('#sea-logout-site').length > 0) && ($('#sea-login-site').length > 0)){
		if(getCookieByName('dfx-auth-token') != "")
            $('#sea-logout-site').show();
        else
			$('#sea-login-site').show();
         }
