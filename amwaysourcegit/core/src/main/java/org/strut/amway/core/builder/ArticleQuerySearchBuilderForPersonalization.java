...
public class ArticleQuerySearchBuilderForPersonalization {
    private static Logger LOGGER = LoggerFactory.getLogger(ArticleQuerySearchBuilderForPersonalization.class);

    public static final int DEFAULT_ARTICLE_PARAM_SIZE = 4;

    public static String buildSearchArticleQuery(ArticlesCriteria articlesCriteria, SlingHttpServletRequest request) {
    ...
    private static String generateQueryByTag(ArticlesCriteria articlesCriteria) {
        if (articlesCriteria != null) {
            ...
            String labelStatement = generateLabelStatement(articlesCriteria.getArticleLabels());
    ...
    private static String generateLabelStatement(List<String> listArticleLabelType) {
        String sParam = "";
        if (CollectionUtils.isNotEmpty(listArticleLabelType)) {
            for (String articleLabelType : listArticleLabelType) {
                if (StringUtils.isNotEmpty(sParam)) {
                    sParam += " ,";
                }
                sParam += SearchStatementConstants.addQuote(articleLabelType);
            }
        }
    ...
    private static String generateTemplateStatement() {
    	...
    	LOGGER.info("Template clause>>"+MessageFormat.format(SearchStatementConstants.STATEMENT_SEARCH_ARTICLE_BY_TEMPLATE_PATTERN,
        		templates));
